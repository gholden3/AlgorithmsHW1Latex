...
even with a very large c, the $\frac{n}{100}!$ is never able to catch up. The limit of f(n)/g(n) as n $\to$ $\infty$ actually approaces $\infty$ which means the right side of the equation is actually  a lower bound ($\Omega$), NOT an ubber bound (O). 
